...
def compute_score_function(solution_str, ground_truth, data_source, extra_info, config=None):
    ...
    if "validation" in data_source or "hcv" in data_source or "ipho" in data_source or "Types_1-5_Full_Dataset" in data_source:
        try:
            if config.debug_print:
                print("DEBUG: About to call math_verify_numerical_answer_simple_relative", solution_str, boxed_list, ground_truth, data_source)
            correct, parsed_sol = basic_utils.math_verify_numerical_answer_simple_relative(
                ...
                use_relative_precision=True, max_relative_error=0.01, 
                mode='relative', relative_to_absolute_threshold=0.0, 
                data_source=data_source, config=config,
                force_config_for_validation=False
            )
            if config.debug_print:
        ...
        if generated_answer is not None:
            ...
            try:
                correct, parsed_sol = basic_utils.math_verify_numerical_answer_simple_relative(
                    ...
                    use_relative_precision=True, max_relative_error=1e-2, 
                    mode='relative', relative_to_absolute_threshold=0, 
                    data_source=data_source, config=config,
                    force_config_for_validation=False
                )
                parsed_sol = parsed_sol[-1]
